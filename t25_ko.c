...
int main(){
	...
	//Impressao
	FILE *arch;
	arch = fopen("t25.txt", "w");

	fprintf(arch,"Disciplina: %s %s\n",codDISC, disc);
	for(r=0;r<nAlunos;r++){
		aux1=0;
		aux2=1;	
		fprintf(arch,"Aluno     : %s %s\n\n", aa[r].codAL, aa[r].nomeAL);	
		fprintf(arch,"%i trabalhos\n", aa[r].quantTRAB);
		j=1;
		for(i=0;i<aa[r].quantTRAB;i++){
			fprintf(arch,"	TRAB %i: %.1lf (peso %.1lf) \n",j, aa[r].notaTRAB[aux1][i], aa[r].notaTRAB[aux2][i]);
			j++;		
		}
		fprintf(arch,"Total pontos dos Trabalhos: %.1lf\n", aa[r].somaTRAB);
		fprintf(arch,"Prova %.1lf (peso %.1lf)\n", aa[r].notaPROVA, aa[r].pesoPROVA);
		fprintf(arch,"Nota final = %.1lf\n", aa[r].soma);		
		fprintf(arch,"\n\n");
	}
	fclose(arch);	

   	printf("\n\n**ARQUIVO TXT SALVO COM SUCESSO NA PASTA ATUAL DESTE ARQUIVO!**\n\n");
}
